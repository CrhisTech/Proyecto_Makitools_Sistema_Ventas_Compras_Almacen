...
export default function NuevaCompra() {
    ...
    const [proveedores, setProveedores] = useState([
        { idProveedor: 1, razonSocial: 'Bosch Perú S.A.C.' },
        { idProveedor: 2, razonSocial: 'Makita Importaciones' }
    ]);
    const [productosCatalogo, setProductosCatalogo] = useState([
        { idProducto: 1, nombre: 'Taladro Percutor 12V' },
        { idProducto: 2, nombre: 'Amoladora Angular 4.5"' }
    ]);
    ...
    const handleProductoChange = (e) => {
        if (e.target.name === 'idProducto') {
            const prod = productosCatalogo.find(p => p.idProducto === parseInt(e.target.value));
